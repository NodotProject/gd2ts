...
# Unit tests for GD2TS conversion issues identified in .ai/issues.md
...
func test_self_in_method_call_arguments():
	var gdscript = """func example() -> void:
	some_function(self.width, self.height, self.padding)"""

	var result = converter.transpile_string(gdscript)

	assert_false(result.contains("self."), "self should be converted to this")
	assert_string_contains(result, "this.width", "First parameter should use this")
	assert_string_contains(result, "this.height", "Second parameter should use this")
	assert_string_contains(result, "this.padding", "Third parameter should use this")

func test_self_assignment_to_property():
	var gdscript = """func _ready() -> void:
	self.counter = 0"""

	var result = converter.transpile_string(gdscript)

	assert_false(result.contains("self."), "self should be converted to this")
	assert_string_contains(result, "this.counter", "Should use this for assignment target")
...
func test_typeof_null_check_converted():
	...
	# Should convert to something like: if (this.counter === null || this.counter === undefined)
	# or simply: if (!this.counter)
...
func test_object_literal_with_method_calls():
	var gdscript = """func setup() -> void:
	var config = {
	...
	assert_string_contains(result, "anchor", "anchor key should be preserved")
	assert_string_contains(result, "mod.UIAnchor.Center", "Enum access should be preserved")
	assert_string_contains(result, "bgFill", "bgFill key should be preserved")
	assert_string_contains(result, "[0.1, 0.1, 0.1]", "Array literal should be preserved")

func test_complex_nested_object_from_parseui():
	var gdscript = """func create() -> void:
	self.rootWidget = ParseUI({
		"type": "Container",
	...
	assert_string_contains(result, '"type"', "type key should be preserved")
	assert_string_contains(result, '"Container"', "Container value should be preserved")
	assert_string_contains(result, '"size"', "size key should be preserved")
	assert_string_contains(result, '"position"', "position key should be preserved")
	assert_string_contains(result, '"anchor"', "anchor key should be preserved")
	assert_string_contains(result, '"children"', "children key should be preserved")

	# Nested object in children array
	assert_string_contains(result, "this.width", "Should use this.width")
	assert_string_contains(result, "this.height", "Should use this.height")
	assert_string_contains(result, "this.padding", "Should use this.padding")

	# Should not have malformed object literals
	assert_false(result.contains("{:"), "Should not have empty colon-only objects")
	assert_false(result.contains(": ,"), "Should not have malformed key-value pairs")
...
func test_simple_counter_ui_update_method():
	var gdscript = """func update() -> void:
	if not self.counterText:
		return
	...
	# Check self conversion
	assert_false(result.contains("self."), "All self references should be converted")
	assert_string_contains(result, "this.counterText", "Should use this.counterText")
	assert_string_contains(result, "this.counter", "Should use this.counter")
